...
class modelling(StateMachineProgram):
    ...
    def update_3d_snapshot(self):
        ...
        if detector is None or image is None or world_map is None:
            print("[SNAPSHOT] Warning: Perception pipeline not active.")
            return
        ...
        try:
            ...
            for obs in observations:
                ...
                # Persist raw face orientation entry
                world_map.objects[obj_id] = domino_obj

                status_str = "FALLEN" if obs.is_fallen else "STANDING"
                print(f"[TM SNAPSHOT] Persistent update [{status_str}] {face_label} -> X={x_mm:.1f}mm, Y={y_mm:.1f}mm")
    ...
    class ReportDominoes(StateNode):
        def start(self, event=None):
            ...
            if not dominoes:
                ...
                print("\n--- Current Persistent WorldMap Dominoes ---")
                for obj in dominoes:
                    print(obj)
            self.post_completion()

    def setup(self):
        intro = Print("\n[READY] Directional domino perception active.\nType 'tm' in terminal for snapshot.").set_name("intro").set_parent(self)
