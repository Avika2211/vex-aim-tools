...
# Model Paths
SEG_MODEL_PATH = os.path.join(PROJECT_DIR, "standing.pt")
CLS_MODEL_PATH = os.path.join(PROJECT_DIR, "standinghalf.pt")
FALLEN_MODEL_PRIMARY = os.path.join(PROJECT_DIR, "fallen.pt")
FALLEN_MODEL_FALLBACK = os.path.join(
    PROJECT_DIR, "runs_play_dominos", "play_dominos_seg_v1_img160", "weights", "standing.pt"
)
...
class kickpickrefine(StateMachineProgram):
    ...
    class ApproachCheckpoint(ActionNode):
        """Drives forward to stop 2 cm (20 mm) before reaching the domino."""
        def start(self, event=None):
            super().start(event)
            target = self.parent.target_data
            if target is None:
            ...
            # Target distance minus 2 cm (20 mm)
    ...
    class TurnRight90(ActionNode):
        """Turns 90 degrees RIGHT."""
        def start(self, event=None):
            super().start(event)
            print("[Knock Sequence 1/4] Turning 90 degrees RIGHT...")
            self.robot.actuators["drive"].turn(self, math.radians(-90.0), None)

    class TurnLeft90(ActionNode):
        """Turns 90 degrees LEFT."""
        def start(self, event=None):
            super().start(event)
            print("[Knock Sequence 2/4] Turning 90 degrees LEFT...")
            self.robot.actuators["drive"].turn(self, math.radians(90.0), None)

    class ForwardKnock13cm(ActionNode):
        """Drives forward 13 cm (130 mm) to knock the domino."""
        def start(self, event=None):
            super().start(event)
            print("[Knock Sequence 3/4] Driving 130.0 mm (13 cm) FORWARD to knock domino...")
            self.robot.actuators["drive"].forward(self, 130.0, None)

    class Retreat4cm(ActionNode):
        """Retreats backward 4 cm (40 mm)."""
        def start(self, event=None):
            super().start(event)
            print("[Knock Sequence 4/4] Driving 40.0 mm (4 cm) BACKWARD...")
            self.parent.actual_retreated_mm = 40.0
            self.robot.actuators["drive"].forward(self, -40.0, None)
    ...
    # ==========================================
    # POST-TURN RE-SCAN & FINE ALIGNMENT NODES
    # ==========================================
    class RescanFallenClose(StateNode):
        """Rescans fallen domino black line at close range after turning."""
        def start(self, event=None):
            super().start(event)
            time.sleep(0.15)
    ...
    class FineTuneSideways(ActionNode):
        """Priority 1: Shifts sideways to align domino center line."""
        def start(self, event=None):
            super().start(event)
            sideways_mm = self.parent.post_turn_sideways_mm
            if abs(sideways_mm) > 1.0:
                print(f"[FINE-TUNE 1/2 - Priority 1] Shifting sideways by {-sideways_mm:.1f}mm...")
                self.robot.actuators["drive"].sideways(self, -sideways_mm, None)
            else:
                self.post_completion()

    class FineTuneParallelAngle(ActionNode):
        """Priority 2: Adjusts angle to bring base slope slope to 0° (perfectly parallel)."""
        def start(self, event=None):
            super().start(event)
            angle_rad = self.parent.post_turn_angle_rad
            angle_deg = math.degrees(angle_rad)
            if abs(angle_deg) > 0.4:
                print(f"[FINE-TUNE 2/2 - Priority 2] Adjusting chassis angle by {-angle_deg:.2f}° to zero base slope...")
                self.robot.actuators["drive"].turn(self, -angle_rad, None)
            else:
                self.post_completion()

    class Step4_Move10cmForward(ActionNode):
        """Step 4: Drives forward into position after fine alignment."""
        def start(self, event=None):
            super().start(event)
    ...
    def setup(self):
        ...
        # Direct Knock & Retreat Sequence Nodes
        turn_right90 = self.TurnRight90().set_name("turn_right90").set_parent(self)
        turn_left90 = self.TurnLeft90().set_name("turn_left90").set_parent(self)
        knock_fwd = self.ForwardKnock13cm().set_name("knock_fwd").set_parent(self)
        retreat_back = self.Retreat4cm().set_name("retreat_back").set_parent(self)
        ...
        # Direct flow from Approach Checkpoint -> Turn 90° Right (No near readjustment)
        CompletionTrans().add_sources(approach_chk).add_destinations(turn_right90)
        FailureTrans().add_sources(approach_chk).add_destinations(wait_scan)

        CompletionTrans().add_sources(turn_right90).add_destinations(turn_left90)
        FailureTrans().add_sources(turn_right90).add_destinations(wait_scan)

        CompletionTrans().add_sources(turn_left90).add_destinations(knock_fwd)
        FailureTrans().add_sources(turn_left90).add_destinations(wait_scan)
        ...
        CompletionTrans().add_sources(retreat_back).add_destinations(wait_after_kick)
        ...
        # Detect -> Step 1 -> Step 2 -> Step 3 (Turn) -> Rescan -> Sideways Align (Priority 1) -> Parallel Angle Align (Priority 2, Slope 0°) -> Step 4 -> Step 5
